...
- Есть хотя бы один "error" ИЛИ missingRequirements непустой → «Требования не выполнены, нужно исправить» + кратко суть/количество. Запрещено писать «выполнено».
...
6. **fix ОБЯЗАТЕЛЕН** — короткое и простое предложение. Для severity=error/warning пиши «исправить на X», «удалить Y», «добавить Z». Для severity=suggestion пиши «можно улучшить, X», «можно вынести в Y» (это рекомендация, не обязательна). НЕ упоминай эталонное решение
...
{{
  "issues": [
    ...
    {{"lineStart": 21, "lineEnd": 21, "codeFragment": "\\"404\\":", "severity": "suggestion", "problem": "Для GET списка ресурсов 404 обычно не нужен — пустой список возвращает 200", "fix": "Можно удалить блок 404 для GET /users"}}
  ],
  "missingRequirements": [],
